...
public class MyHandler extends NotificationsHandler {
    ...
    private void sendNotification(Push msg) {
        ...
        if(msg.getOption().equals("comment")){
            boolean check = sharedPreferences.getBoolean(ctx.getResources().getStringArray(R.array.setting)[1], true);
            if (check){
            ...
            boolean check = sharedPreferences.getBoolean(ctx.getResources().getStringArray(R.array.setting)[0], true);
            if (check){
                mBuilder = new NotificationCompat.Builder(ctx)
                        .setSmallIcon(getNotificationIcon())
                        .setContentTitle(msg.getTitle())
